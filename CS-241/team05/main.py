...
class MyApplication(arcade.Window):
    ...
    def on_draw(self):
        ...
        # Draw fill in square
        arcade.draw_rectangle_filled(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2, self.square_size, self.square_size, arcade.color.BLUE)
    ...
    def on_mouse_press(self, x, y, button, key_modifiers):
        ...
        """
        if button == arcade.MOUSE_BUTTON_LEFT and self.ball_x_pixels_per_second > 0:
            self.ball_x_pixels_per_second += 100

        if button == arcade.MOUSE_BUTTON_LEFT and self.ball_x_pixels_per_second < 0:
            self.ball_x_pixels_per_second -= 100

        print(f'The balls current speed is {self.ball_x_pixels_per_second}')
